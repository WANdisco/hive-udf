...
/**
 * Hive Generic UDAF for approximate counting of unique values in a column
 * (cardinality estimation). This UDAF implements HyperLogLog (see
 * http://algo.inria.fr/flajolet/Publications/FlFuGaMe07.pdf) and Linear
 * Counting (http://dblab.kaist.ac.kr/Publication/pdf/ACM90_TODS_v15n2.pdf).
 * <p>
 * The default is HyperLogLog with bit-size parameter = 16, which should achieve
 * error rates of around 0.5% for reasonably large cardinalities, without too
 * much memory overhead during processing.
 * </p>
 */

@Description(name = "approx_distinct", value = "_FUNC_(x, [t], [b]) - Adds values from x to new Cardinality Estimator, with "
...
public class UDAFCardinalityEstimator implements GenericUDAFResolver2 {
	...
	public enum CardinalityEstimator {
		HLL("HyperLogLog"), LC("LinearCounting");
	...
	static final Log LOG = LogFactory.getLog(UDAFCardinalityEstimator.class
			.getName());
	...
	public GenericUDAFEvaluator getEvaluator(GenericUDAFParameterInfo info)
		...
		if (!(parameters[0].getCategory() == ObjectInspector.Category.PRIMITIVE || parameters[0]
				.getCategory() == ObjectInspector.Category.STRUCT)) {
		...
		// validate the second parameter, which is a String cardinality
		// estimator type.
		if (parameters.length >= 2) {
			...
			if (((PrimitiveTypeInfo) parameters[1]).getPrimitiveCategory() != PrimitiveObjectInspector.PrimitiveCategory.STRING) {
				throw new UDFArgumentTypeException(1,
		...
		if (parameters.length > 4)
			throw new IllegalArgumentException(
					"Function only takes 1, 2 or 3 parameters.");
	...
	/**
	 * Class to evaluate values, and add them to an approximate cardinality
	 * estimator datastructure. Currently HyperLogLog and Linear Counting are
	 * supported.
	 */
	public static class CardinalityEstimatorEvaluator extends
			GenericUDAFEvaluator {
		// defaults to HyperLogLog if no type parameter is present in the
		// function call
		...
		public ObjectInspector init(Mode m, ObjectInspector[] parameters)
			...
			if (m == Mode.PARTIAL1 || m == Mode.COMPLETE) {
				...
				case PRIMITIVE:
					...
					break;
				case STRUCT:
					// if an existing estimator, we don't take the extra args
					// since
					// we can only merge compatible data structures
					inputStructOI = (StructObjectInspector) parameters[0];
					break;
				default:
					// should never happen since we already validated the args
					// in getEvaluator
					throw new IllegalArgumentException(
							"Only PRIMITIVE types and existing STRUCTs "
									+ "containing cardinality estimators are allowed as input. Passed a "
									+ cat.name());
				}
			} else {
		...
		public void iterate(AggregationBuffer aggregationBuffer,
			...
			int param = -1;
			if (parameters.length >= 2) {
				String type = PrimitiveObjectInspectorUtils.getString(
						parameters[1], typeOI);
				ce = CardinalityEstimator.valueOf(type.toUpperCase());
			}
			if (parameters.length == 3) {
				param = PrimitiveObjectInspectorUtils.getInt(parameters[2],
						paramOI);
			}
			...
			if (inputPrimitiveOI != null) {
				...
				LazyString type = (LazyString) inputStructOI
						.getStructFieldData(obj,
								inputStructOI.getStructFieldRef(ESTIMATOR_TYPE));
				LazyBinary lb = (LazyBinary) inputStructOI.getStructFieldData(
						obj, inputStructOI.getStructFieldRef(BINARY));
				ICardinality that = buildEstimator(lb.getWritableObject(),
						type.toString());
		 ...
		 *            current state of the aggregation
		 * @return partial result, in the form of a list of
		 *         {@link BytesWritable} containing the estimator type, and the
		...
		public Object terminatePartial(AggregationBuffer aggregationBuffer)
			...
			try {
				...
				throw new HiveException(
						"Failed to extract byte[] during partial termination. ",
						e);
			}
		}
		 ...
		 *            current state of the aggregation
		 * @param partial
		 *            estimator object to be merged (see
		 *            {@link #terminatePartial(org.apache.hadoop.hive.ql.udf.generic.GenericUDAFEvaluator.AggregationBuffer)}
		 * @throws HiveException
		 */
		@Override
		 ...
		 * @param aggregationBuffer
		 *            current state of the aggregation
		 * @return Hive struct data type, {type: string, cardinality: bigint,
		 *         binary: binary}, containing the type of the estimator, the
		 *         estimated cardinality, and the serialised estimator
		 * @throws HiveException
		 */
		@Override
		public Object terminate(AggregationBuffer aggregationBuffer)
			...
			try {
				...
				long cardinality = ceb.cardinalityEstimator instanceof HyperLogLog ? ((HyperLogLog) ceb.cardinalityEstimator)
						.cardinality() : ceb.cardinalityEstimator.cardinality();
				result.add(new LongWritable(cardinality));
				result.add(new BytesWritable(ceb.cardinalityEstimator
						.getBytes()));
				return result;
			} catch (IOException e) {
				throw new HiveException(
						"Failed to extract serialised Cardinality Estimator instance. ",
						e);
			}
		}
		 ...
		 * 
		 * @param parameter
		 *            <i>b</i> parameter for HyperLogLog or Linear Counting
		 * @param ceb
		 *            the {@link AggregationBuffer} to initialise
		 */
		private void initEstimator(int parameter, CardinalityEstimatorBuffer ceb) {
			switch (ce) {
			case HLL:
				ceb.cardinalityEstimator = new HyperLogLog(
						parameter < 0 ? HLL_DEFAULT_B : parameter);
				break;
			case LC:
				ceb.cardinalityEstimator = new LinearCounting.Builder(
						parameter < 0 ? LC_DEFAULT_SIZE : parameter).build();
				break;
			}
		}

		/**
		 * Builds a cardinality estimator from a serialised binary
		 * representation.
		 * 
		 * @param bw
		 *            byte[] representation (wrapped in a BytesWritable object)
		 * @param type
		 *            one of HLL for HyperLogLog, or LC for Linear Counting
		 * @return cardinality estimator instance
		 * @throws HiveException
		 */
		private ICardinality buildEstimator(BytesWritable bw, String type)
				throws HiveException {
			type = type.trim();
			...
			case HLL:
				try {
					...
					throw new HiveException(
							"Failed to parse byte[] from partial result. ", e);
				}
				break;
			case LC:
			...
			default:
				throw new IllegalArgumentException(
						"Unknown CardinalityEstimator type [" + ce
		...
		/**
		 * Either merge a partial estimator into the current aggregation buffer,
		 * or if the buffer is empty, simply set to the partial estimator
		 * 
		 * @param thatEstimator
		 *            the cardinality estimator to merge in
		 * @param thisEstimatorBuffer
		 *            the current aggregation buffer instance
		 * @throws HiveException
		 */
		private static void mergeEstimators(ICardinality thatEstimator,
				CardinalityEstimatorBuffer thisEstimatorBuffer)
				throws HiveException {
			try {
				if (thisEstimatorBuffer.cardinalityEstimator == null) {
					thisEstimatorBuffer.cardinalityEstimator = thatEstimator;
					LOG.debug("Aggregation buffer is null, using THAT partial instance. Cardinality result = "
							+ thisEstimatorBuffer.cardinalityEstimator
									.cardinality());
				} else {
					LOG.debug("Merging estimator instances, with THIS partial result = "
							+ thisEstimatorBuffer.cardinalityEstimator
									.cardinality()
							+ " and THAT partial result = "
							+ thatEstimator.cardinality());
					thisEstimatorBuffer.cardinalityEstimator = thisEstimatorBuffer.cardinalityEstimator
							.merge(thatEstimator);
					LOG.debug("MERGED partial result = "
							+ thisEstimatorBuffer.cardinalityEstimator
									.cardinality());
				}
			} catch (CardinalityMergeException e) {
				throw new HiveException(
						"Failed to merge Cardinality Estimator instances due to cardinality error. ",
						e);
			}
		}
