...
public class UDAFCntDouble extends AbstractGenericUDAFResolver { //implements GenericUDAFResolver2 {

	static final Log LOG = LogFactory.getLog(UDAFCntDouble.class.getName());
	...
	public static class CountEvaluator extends GenericUDAFEvaluator {
		...
		public void reset(AggregationBuffer aggregationBuffer)
				throws HiveException {
			((CntAggregationBuffer) aggregationBuffer).hash = new TDoubleHashSet();
		}
		...
		public void merge(AggregationBuffer aggregationBuffer, Object partial)
			...
			try {
				...
				hh = (TDoubleHashSet) oi.readObject();
			} catch (Exception e) {
				throw new HiveException(e.getMessage());
			}
		...
		private void mergeHashSets(TDoubleHashSet hh, CntAggregationBuffer ceb) {
			if (ceb.hash.size() == 0) {
				ceb.hash = hh;
				return;
			}
		...
		static class CntAggregationBuffer extends AbstractAggregationBuffer {  //implements AggregationBuffer {
			TDoubleHashSet hash = new TDoubleHashSet();
